...
// Predicate reports whether to pick p. dir is true when p is a directory.
// A nil Predicate accepts everything.
//
// A false file is skipped. A false directory is pruned: descendants
// are not visited.
type Predicate func(p path.Path, dir bool) bool

// Accept reports whether p picks name. A nil Predicate accepts.
func (p Predicate) Accept(name path.Path, dir bool) bool {
	if p == nil {
		return true
	}
	return p(name, dir)
...
// Not inverts p. A nil Predicate inverts to reject everything.
func Not(p Predicate) Predicate {
	return func(name path.Path, dir bool) bool {
		return !p.Accept(name, dir)
	}
}
...
func Prune(pattern string) Predicate {
