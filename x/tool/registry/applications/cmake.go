...
func (t *cmakeTool) ListArtifacts(ctx context.Context, version string) ([]tool.Artifact, error) {
	...
	for _, f := range manifest.Files {
		...
		candidates = append(candidates, tool.Artifact{
			OS:   goos,
			Arch: goarch,
			URL:  url,
	...
	return []tool.Artifact{best}, nil
}

func (t *cmakeTool) InstallArtifact(ctx context.Context, artifact tool.Artifact, destDir string) error {
...
func pickBestCMakeArchive(cands []tool.Artifact) tool.Artifact {
	if len(cands) == 1 {
		return cands[0]
	}
...
func (t *cmakeTool) InstallChecks() []tool.Check {
	return tool.Checks(tool.Binary("cmake"))
}
