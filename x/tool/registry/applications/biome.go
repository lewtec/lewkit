...
func (t *biomeTool) Pin() tool.Pin {
	...
	if pin.ExtractVersion == "" {
		pin.ExtractVersion = `^@biomejs/biome@(?<version>\d+\.\d+\.\d+)$`
	}

	return pin
}

func (t *biomeTool) ListArtifacts(ctx context.Context, version string) ([]tool.Artifact, error) {
	v, err := resolveToolVersion(ctx, version, normalizeBiomeVersion, t.ListVersions)
	if err != nil {
		return nil, err
	}
	...
	out := make([]tool.Artifact, 0, len(arts))
	for _, a := range arts {
...
func (t *biomeTool) InstallArtifact(ctx context.Context, artifact tool.Artifact, destDir string) error {
	at, ok := t.inner.(tool.ArtifactTool)
	if !ok {
		return registry.ErrNoArtifactTool
	}
...
func (t *biomeTool) EnsureBinary(ctx context.Context, version string, cmdName string, destDir string) (string, error) {
	p, err := tool.EnsureBinaryNamed(destDir, cmdName, "biome", "biome", func() error {
...
func selectBiomeArtifact(arts []tool.Artifact, goos, goarch string) *tool.Artifact {
	var best *tool.Artifact
	bestScore := -1
	for i := range arts {
		a := &arts[i]
...
func (t *biomeTool) InstallChecks() []tool.Check {
	return tool.Checks(tool.Binary("biome"))
}
