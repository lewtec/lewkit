...
func TestPinDepsSelfCycle(t *testing.T) {
	s := newPinned(t)
	s.mu.Lock()
	id := s.alloc(s.root, "loop", CPU, nop, false)
	s.pinDeps(id, []ID{id})
...
func TestPinDepsTwoNodeCycle(t *testing.T) {
	s := newPinned(t)
	s.mu.Lock()
	a := s.alloc(s.root, "a", CPU, nop, false)
	b := s.alloc(s.root, "b", CPU, nop, false)
	s.pinDeps(b, []ID{a})
...
func TestPinDepsThreeNodeCycle(t *testing.T) {
	...
	a := s.alloc(s.root, "a", CPU, nop, false)
	b := s.alloc(s.root, "b", CPU, nop, false)
	c := s.alloc(s.root, "c", CPU, nop, false)
	s.pinDeps(b, []ID{a})
	s.pinDeps(c, []ID{b})
	s.pinDeps(a, []ID{c})
	err := s.slots[a].err
...
func TestPinDepsDiamondIsNotACycle(t *testing.T) {
	s := newPinned(t)
	s.mu.Lock()
	root := s.alloc(s.root, "root", CPU, nop, false)
	left := s.alloc(s.root, "left", CPU, nop, false)
	right := s.alloc(s.root, "right", CPU, nop, false)
	join := s.alloc(s.root, "join", CPU, nop, false)
