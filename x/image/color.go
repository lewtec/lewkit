...
// RGB is straight red, green, blue, and alpha, each 0..255. Paint uses this.
type RGB struct {
	Red, Green, Blue, Alpha uint8
}

// BGR is [RGB] packed blue, green, red, alpha.
type BGR struct {
...
// RGB returns the logical red, green, and blue.
func (c BGR) RGB() RGB {
	return RGB{Red: c.Red, Green: c.Green, Blue: c.Blue, Alpha: c.Alpha}
}
...
// RGB returns an opaque red, green, and blue.
func (c CMYK) RGB() RGB {
	black := int(c.Black)
	if black == 255 {
	...
	return RGB{channel(c.Cyan), channel(c.Magenta), channel(c.Yellow), 255}
}
...
func (c HSV) RGB() RGB {
	if c.Saturation == 0 {
		return RGB{c.Value, c.Value, c.Value, 255}
	}
	hue := int(c.Hue % 360)
	region := hue / 60
	rest := hue % 60
	...
	switch region {
	case 0:
		return RGB{uint8(v), uint8(t), uint8(p), 255}
	case 1:
		return RGB{uint8(q), uint8(v), uint8(p), 255}
	case 2:
		return RGB{uint8(p), uint8(v), uint8(t), 255}
	case 3:
		return RGB{uint8(p), uint8(q), uint8(v), 255}
	case 4:
		return RGB{uint8(t), uint8(p), uint8(v), 255}
	default:
		return RGB{uint8(v), uint8(p), uint8(q), 255}
	}
}

// ParseRGB reads #RGB, #RRGGBB, or #RRGGBBAA. The leading # is optional.
func ParseRGB(text string) (RGB, error) {
	text = strings.TrimSpace(text)
	text = strings.TrimPrefix(text, "#")
	if !hex(text) {
		return RGB{}, fmt.Errorf("color %q", text)
	}
	var r, g, b, a uint8
	a = 255
	switch len(text) {
	...
	return RGB{Red: r, Green: g, Blue: b, Alpha: a}, nil
}

// Average is the alpha-weighted mean of opaque pixels. A blank image is black.
func Average(img stdimage.Image) RGB {
	if img == nil {
		return RGB{Alpha: 255}
	}
	bounds := img.Bounds()
