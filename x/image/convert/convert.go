...
// ErrFormat means the bytes are not a PNG, JPEG, ICO, or ICNS this package can read.
var ErrFormat = errors.New("image format")

// LinuxSizes are the Status Notifier pixmap sizes hosts pick from.
var LinuxSizes = []int{16, 22, 24, 32, 48}

// ICOSizes are the PNG entries written into an ICO.
var ICOSizes = []int{16, 24, 32, 48, 64, 128, 256}

// ICNSSpec is one modern ICNS chunk: a four-byte type code and a pixel size.
type ICNSSpec struct {
	Type string
	Size int
}

// ICNSSpecs are PNG-in-ICNS type codes.
var ICNSSpecs = []ICNSSpec{
	{"icp4", 16},
	{"icp5", 32},
	{"icp6", 64},
	{"ic07", 128},
...
func Decode(raw []byte) (image.Image, error) {
	...
	case bytes.HasPrefix(raw, []byte{0x89, 'P', 'N', 'G'}):
		...
		if err != nil {
			return nil, fmt.Errorf("%w: png: %w", ErrFormat, err)
		}
		return img, nil
	case bytes.HasPrefix(raw, []byte{0xff, 0xd8}):
		img, err := jpeg.Decode(bytes.NewReader(raw))
		if err != nil {
			return nil, fmt.Errorf("%w: jpeg: %w", ErrFormat, err)
		}
		return img, nil
	...
	default:
		...
		if err != nil {
			return nil, fmt.Errorf("%w: png, jpeg, ico, or icns", ErrFormat)
		}
		return img, nil
	}
}
...
func Square(src image.Image, size int) (*image.NRGBA, error) {
	if src == nil {
		return nil, fmt.Errorf("%w: nil image", ErrFormat)
	}
	if size <= 0 {
		return nil, fmt.Errorf("%w: size %d", ErrFormat, size)
	}
	return resize(padSquare(src), size), nil
}

// Squares pads src once and returns one square image per size.
func Squares(src image.Image, sizes []int) ([]*image.NRGBA, error) {
	if src == nil {
		return nil, fmt.Errorf("%w: nil image", ErrFormat)
	}
...
// ARGB encodes img as ARGB32, network byte order, straight alpha.
...
func EncodeICO(src image.Image, sizes []int) ([]byte, error) {
	...
	if len(entries) == 0 {
		return nil, fmt.Errorf("%w: ico has no sizes", ErrFormat)
	}
	var buf bytes.Buffer
...
func EncodeICNS(src image.Image, specs []ICNSSpec) ([]byte, error) {
	square := padSquare(src)
	var body bytes.Buffer
	for _, spec := range specs {
		if spec.Size <= 0 || len(spec.Type) != 4 {
			return nil, fmt.Errorf("%w: icns %q %d", ErrFormat, spec.Type, spec.Size)
		}
	...
	if body.Len() == 0 {
		return nil, fmt.Errorf("%w: icns has no sizes", ErrFormat)
	}
...
func decodeICO(raw []byte) (image.Image, error) {
	...
	for i := range count {
		...
		if offset+16 > len(raw) {
			return nil, fmt.Errorf("%w: truncated ico", ErrFormat)
		}
	...
	if len(payload) == 0 {
		return nil, fmt.Errorf("%w: empty ico", ErrFormat)
	}
	if bytes.HasPrefix(payload, []byte{0x89, 'P', 'N', 'G'}) {
		img, err := png.Decode(bytes.NewReader(payload))
		if err != nil {
			return nil, fmt.Errorf("%w: ico png: %w", ErrFormat, err)
		}
		return img, nil
	}
...
func decodeDIB(raw []byte) (image.Image, error) {
	if len(raw) < 40 {
		return nil, fmt.Errorf("%w: short dib", ErrFormat)
	}
	header := binary.LittleEndian.Uint32(raw[0:4])
	if header < 40 || int(header) > len(raw) {
		return nil, fmt.Errorf("%w: dib header %d", ErrFormat, header)
	}
	...
	if width <= 0 || height <= 0 || bitCount != 32 || compression != 0 {
		return nil, fmt.Errorf("%w: dib %dx%d %d-bit compression %d", ErrFormat, width, height, bitCount, compression)
	}
	pixels := raw[header:]
	stride := width * 4
	need := stride * height
	if len(pixels) < need {
		return nil, fmt.Errorf("%w: dib pixels", ErrFormat)
	}
...
func decodeICNS(raw []byte) (image.Image, error) {
	if len(raw) < 8 {
		return nil, fmt.Errorf("%w: short icns", ErrFormat)
	}
	...
	if best == nil {
		return nil, fmt.Errorf("%w: icns has no png", ErrFormat)
	}
	return best, nil
}
