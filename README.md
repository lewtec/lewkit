...
| `x/driver/prelude` | Blank-import. Registers the window backends, sound backends, tray, webview, `x/driver/vulkan`, `x/driver/ndeval`, `x/driver/httpclient`, and `x/driver/fetchurl`. |
...
| `x/driver/sound` | `Open`, `Sinks`. `Open` is an `io.WriteCloser` for one sink. An empty sink is the server default. |
| `x/driver/sound/pulse` | Linux playback through libpulse-simple. `Sink` is the PulseAudio sink name. PipeWire serves that API. |
| `x/driver/sound/winmm` | Windows playback through waveOut. `Sink` is a device index or the endpoint name. An empty sink is `WAVE_MAPPER`. |
| `x/driver/sound/coreaudio` | macOS playback through AudioQueue. `Sink` is a device UID or the display name. |
| `x/driver/sound/mem` | Records PCM. Incompatible unless `LEWKIT_SOUND_MEM` is set. |
